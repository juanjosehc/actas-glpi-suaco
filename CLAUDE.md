...
- **Parpadeo "recarga sin parar" al generar/firmar = Live Reload de Live Server, NO código del repo** (ver `docs/MEJORA_UX_ACTAS_FIRMA.md`). Al clic, el backend escribe en `backend/storage/**` (dentro del workspace) y el Live Server de VS Code ordena recargar la página, pisando la navegación única del frontend. Soluciones: (a) usar el `liveServer.settings.ignoreFiles` de `.vscode/settings.json` (`**/storage/**`, `**/target/**`, `**/*.log`), o (b) servir con un estático sin livereload, p. ej. `cd frontend && python -m http.server 8080` (CORS lo acepta). En producción (sin livereload) no ocurre.
